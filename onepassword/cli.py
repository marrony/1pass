...
class CLI(object):
    ...
    def __init__(self, arguments, stdin=sys.stdin, stdout=sys.stdout,
                 stderr=sys.stderr, getpass=getpass.getpass):
        self.stdin = stdin
        self.stdout = stdout
        self.stderr = stderr
...
class CLIList(CLI):
    def run(self):
        ...
        items = self.keychain._items
        #self.stdout.write("%s\n" % items)
        for item in items.keys():
            #items[item].decrypt_with(self.keychain)
            self.stdout.write("%s\n" % item)


    def argument_parser(self):
        parser = super(CLIList, self).argument_parser()
        return parser
...
class CLIGet(CLI):
    ...
    def argument_parser(self):
        parser = super(CLIGet, self).argument_parser()
